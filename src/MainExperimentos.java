...
public class MainExperimentos {
    ...
    // ===================== CONFIGURAÇÃO =====================
    static final int[] M = {1009, 10007, 100003};                 // tamanhos da tabela (primos)
    static final int[] N = {100_000, 1_000_000, 10_000_000};      // datasets
    static final long[] SEEDS = {42L, 43L, 44L};                  // uma por dataset
    static final String[] HFS   = {"DIV", "MUL", "MIX"};          // funções hash
    static final String[] TIPOS = {"ENC", "LIN", "QUAD", "DUP"};  // tipos de tabela

    static final boolean FORCAR_OA   = true;   // true = roda OA mesmo com carga alta (demora mais)
    static final double  LIMITE_CARGA = 0.85;  // usado se FORCAR_OA == false
    static final int REPS = 3;                 // repetições por cenário
    static final int PASSO_PROGRESSO = 100_000; // prints durante N grandes

    // Total de cenários principais (3 N × 3 M × 3 Hash = 27)
            ...
            for (int ni = 0; ni < N.length; ni++) {
                ...
                for (int m : M) {
                    for (String hf : HFS) {
                        ...
                        // 👉 Cada N × M × Hash é 1 cenário global
                        contador++;
                        ...
                        for (String tipo : TIPOS) {
                            ...
                            // Skip se não forçar OA e carga > limite
